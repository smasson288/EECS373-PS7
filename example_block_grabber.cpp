...
void myCallback(const geometry_msgs::PoseStamped& block_pose){
    blockX = block_pose.pose.position.x;
    blockY = block_pose.pose.position.y;
    ROS_INFO("x: %f; y: %f", blockX , blockY);
    block = true;
}
...
int main(int argc, char** argv) {
 ...
 ros::Subscriber my_subscriber_object= nh.subscribe("block_pose",1,myCallback);
    ...
    tool_pose.pose.position.x= 0.3;
     ...
     tool_pose.pose.position.x = blockX;
        tool_pose.pose.position.y = blockY;
		...
		//move up with block in gripper
        ROS_INFO("requesting plan to move up");
